...
- Pass configured providers to context extraction, text ingestion and imports;
  preserve configured import concurrency and explicit overrides.
- Persist and deduplicate context and imports in the selected project's central
  database. Read central memories in MCP views, graphs, audit and maintenance,
  and persist reinforcement and archive restoration through the same database.
- Keep central and archive full-text indexes consistent with memory writes.
  Repair existing stale or duplicate entries once on database open, without
  changing schema version 5. Replace repeated search-sidecar paths atomically.
...
results, and hybrid search says so loudly when it can't run its semantic leg.

### Fixed

- **Multi-word FTS queries no longer require every term to match.** FTS5
  treats space-separated terms as implicit AND, so the long descriptive
  queries the tool docs encourage (`gnosys_discover "auth JWT session tokens
  refresh"`) returned nothing unless *all* terms matched one memory. All FTS
  surfaces (central-DB `searchFts`/`discoverFts` — which also feed recall and
  federated search — the per-store index, and archive search) now try AND
  first for precision, then retry with OR so partial matches return, ranked
  by BM25 (best-covered memories first). Terms are emitted as quoted phrases,
  which also fixes silent zero-result syntax errors on hyphenated tokens and
  FTS5 keywords; trailing `*` prefix queries still work.
- **Hybrid search no longer degrades to keyword-only silently.** Embeddings
  are only built by `gnosys_reindex`/`gnosys reindex`; until then, hybrid
  mode quietly ran keyword-only and semantic mode returned a generic "no
  results". `gnosys_hybrid_search` now prepends a loud warning when the
  semantic leg is skipped, `gnosys_semantic_search` says exactly why it
  cannot run, and the CLI `hybrid-search` command warns on stderr (stdout
  stays clean for `--json`). The check (`canRunSemantic`) covers the
  central-DB fast path correctly: it requires both stored vectors in
  `gnosys.db` *and* the store-local query embedder, so a machine that only
  syncs the central DB is warned too instead of silently mislabeling
  keyword hits as semantic.
- **`resolveForProject` loads personal/global/optional tiers** so passing
  `projectRoot` no longer disables cross-project memory (explicit
  personal/global writes failed with "No writable store found").

### Known issues (fast-follow)

- Embeddings are still only generated by an explicit reindex, not at write
  time, and Dream Mode does not yet check embedding health.
