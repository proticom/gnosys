...
const suitePath = "test-audit/adversarial/repair-unit-full.json";
...
const draft = read("test-audit/adversarial/current-reconciliation-draft.json");
...
const proofFiles = fs.readdirSync("test-audit/mutations").filter(file => file.startsWith("adversarial.") && file.endsWith(".results.json"));
...
for (const file of fs.readdirSync("test-audit/reviews").filter(file => file.endsWith(".after.json") && file !== "adversarial.after.json")) {
  ...
  for (const row of [...(data.tests || data.actions || []), ...(data.additionalTests || [])]) {
    ...
    if (failing.length) {
      ...
      row.mutationIds = [];
      row.evidence = `Ordinary failure reproduced for ${row.defectId}; see test-audit/adversarial/final-open-ordinary.json. This detects an open defect and does not prove successful behavior.`;
      delete row.proof;
      ...
      row.evidence = `Ordinary case passes at f4b9135 and fails under direct fault(s) ${ids.join(", ")}; exact application restoration passes. See test-audit/mutations/adversarial.*.results.json.`;
    }
  }
  write(fullPath, data);
}
const additions = runtime.filter(test => !mapped.has(key(test))).map(test => {
  ...
  return { ...test, classification: "STRONG", action: expectedFailure ? "expected_failure" : "added", defectId, mutationIds,
    evidence: expectedFailure
      ? `Ordinary failing assertion reproduces ${defectId}; test-audit/adversarial/final-open-ordinary.json records the actual failure. No successful behavior or inverse-repair credit is claimed.`
      : `Measured pass/fail/pass against direct fault(s) ${mutationIds.join(", ")}. The actual failing assertions and restored runs are in test-audit/mutations/adversarial.*.results.json.` };
});
...
const fixRefs = { "D-VSC-002": "13756f2", "D-VSC-004": "e2c08a3", "D-CTX-001": "636e032", "D-CTX-002": "a420e3b",
  "G2-D001": "6a69746", "G2-D002": "29a59b0", "G2-D003": "3021bf5" };
const reasons = {
...
for (const defect of defects) {
  ...
  defect.resolution = { kind, cases, ordinaryEvidence: fixed ? suitePath : "test-audit/adversarial/final-open-ordinary.json",
...
const applicationFixRef = "f4b9135ae350fad937a66f03a9bc3ccfbd8c6906";
write("test-audit/adversarial/acceptance.json", { schemaVersion: 1, applicationFixRef,
  applicationFixCommits: execFileSync("git", ["rev-list", "--reverse", `ec19554..${applicationFixRef}`], { encoding: "utf8" }).trim().split("\n"),
  ordinaryRun: { path: suitePath, sha256: hash(suitePath), applicationDigest: "059ec3ee279146604895d9aba2dbc56f56480450e6c52fe27a2a32db0016b412",
