...
fs.writeFileSync(summaryPath, JSON.stringify({ ref, applicationDigest: before, applicationRestored: true, testSourcesRestored: originals.every(([file, bytes]) => fs.readFileSync(file).equals(bytes)), exactExpectedNames: expectedNames, exitCode: run.status, tests }, null, 2) + "\n");
