...
${defects.map((defect) => `### ${defect.id}: ${defect.title}\n\n${defect.evidence}\n\nExpected: ${defect.expected}\n\nOriginally observed: ${defect.actual}\n\n${acceptance
  ? `Current resolution: ${defect.resolution?.kind || "missing"}. ${defect.resolution?.reason || ""}\n\nCurrent reproduction: \`${defect.reproduction}\`. ${defect.resolution?.kind === "fixed" ? `Fix commits: ${(defect.resolution.fixCommits || []).join(", ")}. Direct fault proofs: ${(defect.resolution.faultMutationIds || []).join(", ")}.` : "Retained as an expected failure; ordinary failure evidence establishes the unresolved defect."}\n\nCurrent evidence: ${defect.resolution?.ordinaryEvidence ? link(defect.resolution.ordinaryEvidence) : "missing"}.`
...
The initial F37 inventory incorrectly attributed socket-server read/update operations to the generated helper. Documentation and public exports establish its actual API. The original inventory and exact correction are preserved in ${link("test-audit/features.original.json")} and ${link("test-audit/inventory-corrections.json")}. Final mapping corrections are recorded in ${link("test-audit/feature-map-corrections.json")}.
